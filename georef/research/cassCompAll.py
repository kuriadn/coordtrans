...
for i in range(0, len(lo)):
  ...
  CX.append(x)
  CY.append(y)
  fo.write('{4:6.2f}, {5:6.2f}, {0:9.1f}, {1:9.1f}, {2:9.1f}, {3:9.1f}, {6:9.2f}\n'.format(cx[i],CX[i], cy[i], CY[i], lo[i], la[i], ((cx[i] - CX[i])**2 + (cy[i] - CY[i])**2)**0.5))
